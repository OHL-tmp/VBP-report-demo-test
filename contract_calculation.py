...
def Contract_Calculation(Recom_Contract, UD_Measure,UD_Contract,UD_Pt_Cohort,Rebate_noVBC_flat, Rebate_VBC_flat,Rebate_noVBC_table,Rebate_VBC_table, MarketShare_table,Entresto_Utilizer_Perc,Script_PMPM, Performance_assumption,Recom_Measure_all):  
...
#	Recom_Measure_all=pd.read_csv('data/recom_measure.csv')
	...
	if Rebate_noVBC_table.shape[0]==0:
		...
		for i in range(len(Rebate_noVBC_table)):
	...
	if Rebate_VBC_table.shape[0]==0:
		...
		for i in range(len(Rebate_VBC_table)):
	...
	for i in Type:
		...
		Merge_name[i]=Measure_name[i].merge(Performance_assumption_name[i],left_on='Measure', right_on='Measure', suffixes=('x', 'y'),how='right')
		Merge_name[i]['Weight'].fillna(0,inplace=True)
		Merge_name[i]['Target'].fillna(0.001,inplace=True)  
	...
	NoVBC_Cost=Merge_name['UD'][Merge_name['UD']['Measure']=='CHF Related Average Cost per Patient'][['total_Mid_noVBC', 'total_Worst_noVBC','total_Best_noVBC','total_Worse_noVBC','total_Better_noVBC']]
	
#    RecomVBC_Cost=Merge_name['Recom'][Merge_name['Recom']['Measure']=='CHF Related Average Cost per Patient'][['total_Mid_VBC', 'total_Worst_VBC','total_Best_VBC','total_Worse_VBC','total_Better_VBC']]
	UDVBC_Cost=Merge_name['UD'][Merge_name['UD']['Measure']=='CHF Related Average Cost per Patient'][['total_Mid_VBC', 'total_Worst_VBC','total_Best_VBC','total_Worse_VBC','total_Better_VBC']]
...
#    UDVBC_Cost=Merge_name['UD'][['total_Mid_VBC', 'total_Worst_VBC','total_Best_VBC','total_Worse_VBC','total_Better_VBC']].iloc[0]
	...
	data = {'Scenario': ['Best Estimate', 'Worst', 'Best', 'Lower End', 'Higher End'], 
				...
				'NoVBC Total Cost':[x * Baseline.iat[0,1]/1000000 for x in NoVBC_Cost.iloc[0]],
				'NoVBC Rebate Adjustment':[0,0,0,0,0],
				'NoVBC Total Cost (After Rebate Adj)':[x * Baseline.iat[0,1]/1000000 for x in NoVBC_Cost.iloc[0]],
	...
	Output_Medical_Cost['UDVBC Total Cost']=[x * Baseline.iat[0,1]/1000000 for x in UDVBC_Cost.iloc[0]]
